...
void GetGenre(const char* filename, vector<vector<string>> &vect) {
	...
	if (!f.is_open()) {
		...
		while (!f.eof()) {
			// Iterate through file to find ** START GENRES ** discarding all lines previous
			string line;
			getline(f, line);
			if (line == "** START GENRES **") {
				// Once ** START GENRES ** found, iterate through all subsequent lines
				while (!f.eof()) {
					vect.push_back({});
					getline(f, line);
					size_t pos = 0;
